...
import {BACKEND} from '@/presentation/util'
...
const BackendList = () => {
  ...
  const requestMoreTeam = () => {
    if (data != null && data.length >= individualsFindState.pageNum) {
      dispatch(
        findIndividuals(individualsFindState.pageFrom, individualsFindState.pageNum, BACKEND),
      )
  ...
  useEffect(() => {
    console.log(`첫 렌더링`)
    dispatch(findIndividuals(individualsFindState.pageFrom, individualsFindState.pageNum, BACKEND))
    setIndividualsFindState(prevState => ({
      ...prevState,
      pageFrom: individualsFindState.pageFrom + 1,
    }))
