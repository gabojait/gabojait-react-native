...
export const usernameRegex = /^(?=.*\d)(?=.*[a-z])[a-z\d]{5,15}$/; //5~15자 소문자 영문, 숫자 조합
export const passwordRegex = /^(?=.*[a-zA-Z])(?=.*\d)(?=.*[#$@!%&*?])[a-zA-Z\d#$@!%&*?]{10,30}$/; //영문, 숫자,특수문자(#$@!%&*?) 조합 10~30자
export const nicknameRegex = /^[ㄱ-ㅎ|ㅏ-ㅣ|가-힣]{2,8}$/; //한글 2~8자
