...
const SymbolCenteredModalContent: React.FC<SymbolModalContentProps> = props => {
  ...
  return (
    <View style={[{display: 'flex'}, style.modalContainer]}>
      ...
      <Text style={{textAlign: 'center',paddingVertical:16,fontSize:11, fontWeight:'bold', lineHeight:25}}>{props.text}</Text>
      {props.symbol}
      {props.yesButton ? <FilledButton {...props.yesButton} containerStyle={{marginVertical:10}}/> : null}
    </View>
  )
}
