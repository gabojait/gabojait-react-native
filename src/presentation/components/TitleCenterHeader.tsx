...
const TitleCenterHeader: React.FC<HeaderProps> = ({
    title,
    canGoBack,
    leftChildren,
    }) => {
    ...
    return (
        <View style={[headerStyle.parent, {borderColor: theme.colors.disabled}]}>
            <View style={headerStyle.left}>
                {canGoBack ? back : null}
                {leftChildren}
                <Text style={{fontWeight:theme.fontWeight.semibold, fontSize:theme.fontSize.lg,textAlign:'center', flex:1, marginRight:25}}>{title}</Text>
            </View>
        </View>
    )
    }
